...
//Переменная для хранения пути удаления файла
AnsiString DelFile = "";

//Переменная для хранения прогресса прохождения курса
int Final_Ball = 0;

//Переменная для хранения временного имени пользователя
AnsiString Temporary_Login = "";

//Переменная для хранения временного пароля пользователя
...
//Активный пароль пользователя
AnsiString User_Password = "";


//Создаем объект TStringList для сканирования папки с пользователями
TStringList *Update = new TStringList;
TSearchRec sr;

//Сброс курса по умолчанию
...
void __fastcall TForm1::Account_Deleting(TObject *Sender)
{
	if(Form1->edtConfirmation->Text == User_Password)
	{
		...
		btnDeleteYourAccount->Caption = "Удалить учетную запись";
	}

}

//Сканирование папки на файлы пользователей
void Scanning()
{
	//Получаем количество файлов в папке пользователей
 ...
 {
	//Проверяем, что введеные логин и пароль не пустые
	if((Temporary_Login != "")&&(Temporary_Password != ""))
	{
		if(Form1->lbList_Login->Count > 0)
		{
			//Если кол - во итемов больше 0, запускаем цикл
			int Login_Verification = 0;
			for(int i = 0; i <= Form1->lbList_Login->Count-1; i++)
			{
				//Если есть такой логин, то не разрешаем создание нового аккаунта
		...
		{
			//Если кол - во итемов равно 0, то создаем новый аккаунт
...
{
	//Создаем объект Ini конфигурации Имя пользователя.ini для работы с пользователем
	if((Login != "")&&(Password != ""))
	{
		//Создаем объект Ini конфигурации Имя пользователя.ini для работы с пользователем
		TIniFile *Ini = new TIniFile(ExtractFilePath(Application->ExeName)+L"\\Users" + L"\\" + Login + L".ini");

		//Заносим в файл логин пользователя
		Ini->WriteString("User", "Login", Login);

		//Заносим в файл пароль пользователя
		Ini->WriteString("User", "Password", Password);

		//Заносим в файл остальные настройки
...
void __fastcall TForm1::Login_Status(TObject *Sender)
{
	if((User_Login != "")&&(User_Password != ""))
	{
		...
		//Разблокируем темы для изучения
	...
	{
		//Сообщаем, что вход не выполнен
		lbSV_Settings_Account_Name->Caption = "Вход не выполнен";
		//Запрещаем удалять учетную запись
		btnDeleteYourAccount->Enabled = false;
		//Запрещаем сброс курса
		btnResetDefaultCourse->Enabled = false;
		//Запрещаем изменение пароля
		btnSV_Settings_Password_Reset->Enabled = false;
		//Меняем действие кнопки выход на вход
		btnSV_Setting_Profile_Exit->Caption = "Войти";
		//Включаем отображение индикатора
		...
		//Отключаем иконку профиля
		imgProfile_Settings->Visible = false;
		//Выключаем возможность изменения настроек пользователем
		...
		for(int i = 0; i <= Form1->lbList_Login->Count-1; i++)
		{
			...
			if(Validation == Temporary_Login)
			{
				...
				else if(Validation_Password != Temporary_Password)
				{
					...
					edValidation_Password_01->TextHint = "Неверный пароль!";
			...
			else if(Validation != Temporary_Login)
			{
				...
				edValidation_Login_01->TextHint = "Неверный логин!";
...
void __fastcall TForm1::Change_Password(TObject *Sender)
{
	if(Form1->edReset_Password->Text == User_Password)
	{
		if(Form1->edNew_Password_01->Text == Form1->edNew_Password_02->Text)
		{
			...
			btnSV_Settings_Password_Reset->Caption = "Изменить пароль";
			WriteParams(Sender);
		}
	}
}
...
{
	//Адаптация приложения под размер экрана
	...
	{
		/*if(mrOk == MessageDlg("Внимание! Программа не поддерживает масштаб экрана ниже соотношения 1600 x 900. Приложение будет закрыто", mtInformation,TMsgDlgButtons() <<mbOK, 0))
		{
	...
	//Переводим главное меню в компактный режим
	SV->Close();
	//Переводим меню настроек в скрытый режим
	SV_Settings->Close();
	//Переводим системное меню в скрытый режим
	pnlTopControl->Height = 1;
	//Выводим панель Home на главный экран
	pnlHome->BringToFront();
	pnlHome->Enabled = true;
	pnlHome->Visible = true;
	//Переводим окно входа в активный режим
	pnlValidation->Visible = true;
	//Сканируем папку пользователей
	Scanning();
	//Проверяем статус входа в систему
	Login_Status(Sender);
...
{
	//Управление состоянием главного меню
	SV->BringToFront();
	if(SV->Opened)
	{
		SV->Close();
...
void __fastcall TForm1::actExitExecute(TObject *Sender)
{
	...
	{
		//Сохраняем параметры учетной записи
		WriteParams(Sender);
	}
...
{
	//Управление состоянием меню настроек
	...
	{
		//Закрываем главное меню если выбран пункт в настройках
		SV->Close();
	}
}
//---------------------------------------------------------------------------
...
{   //Прячем все панели кроме Profile
...
{   //Прячем все панели кроме Training
	pnlHome->Enabled = false;
	pnlHome->Visible = false;
	pnlProfile->Enabled = false;
	pnlProfile->Visible = false;
...
{
	//Прячем все панели кроме Statistics
	pnlHome->Enabled = false;
	pnlHome->Visible = false;
	pnlProfile->Enabled = false;
	pnlProfile->Visible = false;
	...
	if((Final_Ball > 0)&&(Final_Ball < 40))
	{
		...
		pnlValue_1->Caption = "Ваша оценка: 1";
	...
	if((Final_Ball >= 40)&&(Final_Ball < 60))
	{
		...
		pnlValue_2->Caption = "Ваша оценка: 2";
		pnlValue_3->Color = clBlack;
		pnlValue_4->Color = clBlack;
		pnlValue_5->Color = clBlack;
	}
	if((Final_Ball >= 60)&&(Final_Ball < 80))
	{
		...
		pnlValue_3->Caption = "Ваша оценка: 3";
		pnlValue_4->Color = clBlack;
		pnlValue_5->Color = clBlack;
	}
	if((Final_Ball >= 80)&&(Final_Ball < 100))
	{
		...
		pnlValue_4->Caption = "Ваша оценка: 4";
		pnlValue_5->Color = clBlack;
	}
	if(Final_Ball == 100)
	{
		...
		pnlValue_5->Caption = "Ваша оценка: 5";
	}
}
//---------------------------------------------------------------------------
...
void __fastcall TForm1::actHelpExecute(TObject *Sender)
{
	...
	//Запускаем просмотр помощи
	ShellExecute(Handle,
	L"open", (ExtractFilePath(Application->ExeName) + L"Help\\Руководство пользователя.pdf").w_str(),
...
{
	//Активация системной панели TopControl
	pnlTopControl->BringToFront();
	if(pnlTopControl->Height == 1)
	{
		//Открываем системную панель
...
void __fastcall TForm1::N1Click(TObject *Sender)
{
	...
	pnlAbout->Enabled = false;
	pnlAbout->Visible = false;
	//Вызываем панель Help
	pnlHelp->BringToFront();
	pnlHelp->Enabled = true;
	pnlHelp->Visible = true;
	//Закрываем системную панель
	pnlTopControl->Height = 1;
}
//---------------------------------------------------------------------------
...
void __fastcall TForm1::N2Click(TObject *Sender)
{
	//Закрываем системную панель
	pnlTopControl->Height = 1;
	//Вызываем меню настроек SV_Settings
	SV_Settings->BringToFront();
	SV_Settings->Open();
}
//---------------------------------------------------------------------------
...
void __fastcall TForm1::tsSV_Settings_Status_03Click(TObject *Sender)
{
	...
	{
		//Переводим приложение в оконный режим
		Form1->Width = 1000;
		Form1->Height = 700;
...
void __fastcall TForm1::btnValidation_Entrance_01Click(TObject *Sender)
{
	...
	//Отправляем на проверку логин
	Temporary_Login = edValidation_Login_01->Text;

	 //Отправляем на проверку пароль
...
void __fastcall TForm1::lbRegistrationClick(TObject *Sender)
{
	...
	//Проверяем на доступность, и если доступен, то создаем
...
{
	if(btnSV_Setting_Profile_Exit->Caption == "Выйти")
	{
	...
	else if(btnSV_Setting_Profile_Exit->Caption == "Войти")
	{
		//Прячем все панели кроме Profile
		pnlHome->Enabled = false;
		pnlHome->Visible = false;
...
void __fastcall TForm1::btnSV_Settings_Password_ResetClick(TObject *Sender)
{
	...
	{
		btnSV_Settings_Password_Reset->Caption = "Отменить";
...
{
	//Загрузка текста для первого задания
	Ric_01->Lines->LoadFromFile(ExtractFilePath(Application->ExeName)+L"\\Library"+L"\\Глава1_Занятие_1.rtf");
...
{
	//Загрузка текста для первого задания
	Ric_02->Lines->LoadFromFile(ExtractFilePath(Application->ExeName)+L"\\Library"+L"\\Глава1_Занятие_2.rtf");
	//Блок управления цветом
	pnlTopic_01->Color = clMenu;
	pnlTopic_03->Color = clMenu;
	pnlTopic_04->Color = clMenu;
...
{
	//Загрузка текста для первого задания
	Ric_03->Lines->LoadFromFile(ExtractFilePath(Application->ExeName)+L"\\Library"+L"\\Глава1_Занятие_3.rtf");
	//Блок управления цветом
	pnlTopic_01->Color = clMenu;
	pnlTopic_02->Color = clMenu;
	pnlTopic_04->Color = clMenu;
...
{
	//Загрузка текста для первого задания
	Ric_04->Lines->LoadFromFile(ExtractFilePath(Application->ExeName)+L"\\Library"+L"\\Глава1_Занятие_4.rtf");
	//Блок управления цветом
	pnlTopic_01->Color = clMenu;
	pnlTopic_02->Color = clMenu;
	pnlTopic_03->Color = clMenu;
...
{
	//Загрузка текста для первого задания
	Ric_05->Lines->LoadFromFile(ExtractFilePath(Application->ExeName)+L"\\Library"+L"\\Глава1_Занятие_5.rtf");
	//Блок управления цветом
	pnlTopic_01->Color = clMenu;
	pnlTopic_02->Color = clMenu;
	pnlTopic_03->Color = clMenu;
...
{
	//Загрузка текста для первого задания
	Ric_06->Lines->LoadFromFile(ExtractFilePath(Application->ExeName)+L"\\Library"+L"\\Глава1_Занятие_6.rtf");
	//Блок управления цветом
	pnlTopic_01->Color = clMenu;
	pnlTopic_02->Color = clMenu;
	pnlTopic_03->Color = clMenu;
...
{
	//Загрузка текста для первого задания
	Ric_07->Lines->LoadFromFile(ExtractFilePath(Application->ExeName)+L"\\Library"+L"\\Глава1_Занятие_7.rtf");
	//Блок управления цветом
	pnlTopic_01->Color = clMenu;
	pnlTopic_02->Color = clMenu;
	pnlTopic_03->Color = clMenu;
...
{
	//Загрузка текста для первого задания
	Ric_08->Lines->LoadFromFile(ExtractFilePath(Application->ExeName)+L"\\Library"+L"\\Глава1_Занятие_8.rtf");
	//Блок управления цветом
	pnlTopic_01->Color = clMenu;
	pnlTopic_02->Color = clMenu;
	pnlTopic_03->Color = clMenu;
...
{
	//Загрузка текста для первого задания
	Ric_09->Lines->LoadFromFile(ExtractFilePath(Application->ExeName)+L"\\Library"+L"\\Глава1_Занятие_9.rtf");
	//Блок управления цветом
	pnlTopic_01->Color = clMenu;
	pnlTopic_02->Color = clMenu;
	pnlTopic_03->Color = clMenu;
...
{
	//Загрузка текста для первого задания
	Ric_10->Lines->LoadFromFile(ExtractFilePath(Application->ExeName)+L"\\Library"+L"\\Глава1_Занятие_10.rtf");
	//Блок управления цветом
	pnlTopic_01->Color = clMenu;
	pnlTopic_02->Color = clMenu;
	pnlTopic_03->Color = clMenu;
...
{
	//Загрузка текста для первого задания
	Ric_11->Lines->LoadFromFile(ExtractFilePath(Application->ExeName)+L"\\Library"+L"\\Глава2_Занятие_1.rtf");
	//Блок управления цветом
	pnlTopic_01->Color = clMenu;
	pnlTopic_02->Color = clMenu;
	pnlTopic_03->Color = clMenu;
...
{
	//Загрузка текста для первого задания
	Ric_12->Lines->LoadFromFile(ExtractFilePath(Application->ExeName)+L"\\Library"+L"\\Глава2_Занятие_2.rtf");
	//Блок управления цветом
	pnlTopic_01->Color = clMenu;
	pnlTopic_02->Color = clMenu;
	pnlTopic_03->Color = clMenu;
...
{
	//Загрузка текста для первого задания
    Ric_13->Lines->LoadFromFile(ExtractFilePath(Application->ExeName)+L"\\Library"+L"\\Глава2_Занятие_3.rtf");
	//Блок управления цветом
	pnlTopic_01->Color = clMenu;
	pnlTopic_02->Color = clMenu;
	pnlTopic_03->Color = clMenu;
...
{
	//Загрузка текста для первого задания
	Ric_14->Lines->LoadFromFile(ExtractFilePath(Application->ExeName)+L"\\Library"+L"\\Глава2_Занятие_4.rtf");
	//Блок управления цветом
	pnlTopic_01->Color = clMenu;
	pnlTopic_02->Color = clMenu;
	pnlTopic_03->Color = clMenu;
...
{
	//Загрузка текста для первого задания
	Ric_15->Lines->LoadFromFile(ExtractFilePath(Application->ExeName)+L"\\Library"+L"\\Глава2_Занятие_5.rtf");
	//Блок управления цветом
	pnlTopic_01->Color = clMenu;
	pnlTopic_02->Color = clMenu;
	pnlTopic_03->Color = clMenu;
...
{
	//Загрузка текста для первого задания
	Ric_16->Lines->LoadFromFile(ExtractFilePath(Application->ExeName)+L"\\Library"+L"\\Глава2_Занятие_6.rtf");
	//Блок управления цветом
	pnlTopic_01->Color = clMenu;
	pnlTopic_02->Color = clMenu;
	pnlTopic_03->Color = clMenu;
...
{
	//Загрузка текста для первого задания
	Ric_17->Lines->LoadFromFile(ExtractFilePath(Application->ExeName)+L"\\Library"+L"\\Глава2_Занятие_7.rtf");
	//Блок управления цветом
	pnlTopic_01->Color = clMenu;
	pnlTopic_02->Color = clMenu;
	pnlTopic_03->Color = clMenu;
...
{
	//Загрузка текста для первого задания
	Ric_18->Lines->LoadFromFile(ExtractFilePath(Application->ExeName)+L"\\Library"+L"\\Глава2_Занятие_8.rtf");
	//Блок управления цветом
	pnlTopic_01->Color = clMenu;
	pnlTopic_02->Color = clMenu;
	pnlTopic_03->Color = clMenu;
...
{
	//Загрузка текста для первого задания
    Ric_19->Lines->LoadFromFile(ExtractFilePath(Application->ExeName)+L"\\Library"+L"\\Глава2_Занятие_9.rtf");
	//Блок управления цветом
	pnlTopic_01->Color = clMenu;
	pnlTopic_02->Color = clMenu;
	pnlTopic_03->Color = clMenu;
...
{
	//Загрузка текста для первого задания
	Ric_20->Lines->LoadFromFile(ExtractFilePath(Application->ExeName)+L"\\Library"+L"\\Глава3_Занятие_1.rtf");
	//Блок управления цветом
	pnlTopic_01->Color = clMenu;
	pnlTopic_02->Color = clMenu;
	pnlTopic_03->Color = clMenu;
...
{
	//Загрузка текста для первого задания
	Ric_21->Lines->LoadFromFile(ExtractFilePath(Application->ExeName)+L"\\Library"+L"\\Глава3_Занятие_2.rtf");
	//Блок управления цветом
	pnlTopic_01->Color = clMenu;
	pnlTopic_02->Color = clMenu;
	pnlTopic_03->Color = clMenu;
...
{
	//Загрузка текста для первого задания
	Ric_22->Lines->LoadFromFile(ExtractFilePath(Application->ExeName)+L"\\Library"+L"\\Глава3_Занятие_3.rtf");
	//Блок управления цветом
	pnlTopic_01->Color = clMenu;
	pnlTopic_02->Color = clMenu;
	pnlTopic_03->Color = clMenu;
...
{
	//Загрузка текста для первого задания
	Ric_23->Lines->LoadFromFile(ExtractFilePath(Application->ExeName)+L"\\Library"+L"\\Глава3_Занятие_4.rtf");
	//Блок управления цветом
	pnlTopic_01->Color = clMenu;
	pnlTopic_02->Color = clMenu;
	pnlTopic_03->Color = clMenu;
...
{
	//Загрузка текста для первого задания
	Ric_24->Lines->LoadFromFile(ExtractFilePath(Application->ExeName)+L"\\Library"+L"\\Глава3_Занятие_4.rtf");
	//Блок управления цветом
	pnlTopic_01->Color = clMenu;
	pnlTopic_02->Color = clMenu;
	pnlTopic_03->Color = clMenu;
...
{
	//Загрузка текста для первого задания
	Ric_25->Lines->LoadFromFile(ExtractFilePath(Application->ExeName)+L"\\Library"+L"\\Глава3_Занятие_6.rtf");
	//Блок управления цветом
	pnlTopic_01->Color = clMenu;
	pnlTopic_02->Color = clMenu;
	pnlTopic_03->Color = clMenu;
...
{
	//Загрузка текста для первого задания
	Ric_26->Lines->LoadFromFile(ExtractFilePath(Application->ExeName)+L"\\Library"+L"\\Глава3_Занятие_7.rtf");
	//Блок управления цветом
	pnlTopic_01->Color = clMenu;
	pnlTopic_02->Color = clMenu;
	pnlTopic_03->Color = clMenu;
...
{
	//Загрузка текста для первого задания
	Ric_27->Lines->LoadFromFile(ExtractFilePath(Application->ExeName)+L"\\Library"+L"\\Глава3_Занятие_8.rtf");
	//Блок управления цветом
	pnlTopic_01->Color = clMenu;
	pnlTopic_02->Color = clMenu;
	pnlTopic_03->Color = clMenu;
...
{
	//Загрузка текста для первого задания
	Ric_28->Lines->LoadFromFile(ExtractFilePath(Application->ExeName)+L"\\Library"+L"\\Глава4_Занятие_1.rtf");
	//Блок управления цветом
	pnlTopic_01->Color = clMenu;
	pnlTopic_02->Color = clMenu;
	pnlTopic_03->Color = clMenu;
...
{
	//Загрузка текста для первого задания
	Ric_29->Lines->LoadFromFile(ExtractFilePath(Application->ExeName)+L"\\Library"+L"\\Глава4_Занятие_2.rtf");
	//Блок управления цветом
	pnlTopic_01->Color = clMenu;
	pnlTopic_02->Color = clMenu;
	pnlTopic_03->Color = clMenu;
...
{
	//Загрузка текста для первого задания
	Ric_30->Lines->LoadFromFile(ExtractFilePath(Application->ExeName)+L"\\Library"+L"\\Глава4_Занятие_3.rtf");
	//Блок управления цветом
	pnlTopic_01->Color = clMenu;
	pnlTopic_02->Color = clMenu;
	pnlTopic_03->Color = clMenu;
...
{
	//Загрузка текста для первого задания
	Ric_31->Lines->LoadFromFile(ExtractFilePath(Application->ExeName)+L"\\Library"+L"\\Глава4_Занятие_4.rtf");
	//Блок управления цветом
	pnlTopic_01->Color = clMenu;
	pnlTopic_02->Color = clMenu;
	pnlTopic_03->Color = clMenu;
...
{
	//Загрузка текста для первого задания
	Ric_32->Lines->LoadFromFile(ExtractFilePath(Application->ExeName)+L"\\Library"+L"\\Глава4_Занятие_5.rtf");
	//Блок управления цветом
	pnlTopic_01->Color = clMenu;
	pnlTopic_02->Color = clMenu;
	pnlTopic_03->Color = clMenu;
...
{
	//Загрузка текста для первого задания
	Ric_33->Lines->LoadFromFile(ExtractFilePath(Application->ExeName)+L"\\Library"+L"\\Глава4_Занятие_5.rtf");
	//Блок управления цветом
	pnlTopic_01->Color = clMenu;
	pnlTopic_02->Color = clMenu;
	pnlTopic_03->Color = clMenu;
...
{
	//Загрузка текста для первого задания
	Ric_34->Lines->LoadFromFile(ExtractFilePath(Application->ExeName)+L"\\Library"+L"\\Глава4_Занятие_7.rtf");
	//Блок управления цветом
	pnlTopic_01->Color = clMenu;
	pnlTopic_02->Color = clMenu;
	pnlTopic_03->Color = clMenu;
...
{
	//Загрузка текста для первого задания
	Ric_35->Lines->LoadFromFile(ExtractFilePath(Application->ExeName)+L"\\Library"+L"\\Глава4_Занятие_8.rtf");
	//Блок управления цветом
	pnlTopic_01->Color = clMenu;
	pnlTopic_02->Color = clMenu;
	pnlTopic_03->Color = clMenu;
...
{
	//Загрузка текста для первого задания
    Ric_36->Lines->LoadFromFile(ExtractFilePath(Application->ExeName)+L"\\Library"+L"\\Глава4_Занятие_9.rtf");
	//Блок видимости панелей
	pnlTopic_Selected_01->Visible = false;
	pnlTopic_Selected_02->Visible = false;
	pnlTopic_Selected_03->Visible = false;
...
{
	//Запускаем просиотор видео
	ShellExecute(Handle,
	L"open", (ExtractFilePath(Application->ExeName) + L"Video\\Chapters\\Chapter_1\\Что такое логика.mp4").w_str(),
	NULL, NULL, SW_SHOWNORMAL);
}
//---------------------------------------------------------------------------
...
void __fastcall TForm1::Imag_02Click(TObject *Sender)
{
	//Запускаем просиотор видео
	ShellExecute(Handle,
	L"open", (ExtractFilePath(Application->ExeName) + L"Video\\Chapters\\Chapter_1\\Что такое математика.mp4").w_str(),
	NULL, NULL, SW_SHOWNORMAL);
}
//---------------------------------------------------------------------------
...
void __fastcall TForm1::Imag_03Click(TObject *Sender)
{
	//Запускаем просиотор видео
	ShellExecute(Handle,
	L"open", (ExtractFilePath(Application->ExeName) + L"Video\\Chapters\\Chapter_1\\Софизмы и парадоксы.mp4").w_str(),
	NULL, NULL, SW_SHOWNORMAL);
}
//---------------------------------------------------------------------------
...
void __fastcall TForm1::Imag_04Click(TObject *Sender)
{
	//Запускаем просиотор видео
	ShellExecute(Handle,
	L"open", (ExtractFilePath(Application->ExeName) + L"Video\\Chapters\\Chapter_1\\Математическая логика.mp4").w_str(),
	NULL, NULL, SW_SHOWNORMAL);
}
//---------------------------------------------------------------------------
...
void __fastcall TForm1::Imag_05Click(TObject *Sender)
{
	//Запускаем просиотор видео
	ShellExecute(Handle,
	L"open", (ExtractFilePath(Application->ExeName) + L"Video\\Chapters\\Chapter_1\\Становление логики.mp4").w_str(),
	NULL, NULL, SW_SHOWNORMAL);
}
//---------------------------------------------------------------------------
...
void __fastcall TForm1::Imag_06Click(TObject *Sender)
{
	//Запускаем просиотор видео
	ShellExecute(Handle,
	L"open", (ExtractFilePath(Application->ExeName) + L"Video\\Chapters\\Chapter_1\\Начало математической логики.mp4").w_str(),
	NULL, NULL, SW_SHOWNORMAL);
}
//---------------------------------------------------------------------------
...
void __fastcall TForm1::Imag_07Click(TObject *Sender)
{
	//Запускаем просиотор видео
	ShellExecute(Handle,
	L"open", (ExtractFilePath(Application->ExeName) + L"Video\\Chapters\\Chapter_1\\Математическая логика в своем блеске и великолепии.mp4").w_str(),
	NULL, NULL, SW_SHOWNORMAL);
}
//---------------------------------------------------------------------------
...
void __fastcall TForm1::Imag_11Click(TObject *Sender)
{
	//Запускаем просиотор видео
	ShellExecute(Handle,
	L"open", (ExtractFilePath(Application->ExeName) + L"Video\\Chapters\\Chapter_2\\Интуитивная теория множеств.mp4").w_str(),
	NULL, NULL, SW_SHOWNORMAL);
}
//---------------------------------------------------------------------------
...
void __fastcall TForm1::Imag_12Click(TObject *Sender)
{
	//Запускаем просиотор видео
	ShellExecute(Handle,
	L"open", (ExtractFilePath(Application->ExeName) + L"Video\\Chapters\\Chapter_2\\Операции над множествами.mp4").w_str(),
	NULL, NULL, SW_SHOWNORMAL);
}
//---------------------------------------------------------------------------
...
void __fastcall TForm1::Imag_13Click(TObject *Sender)
{
	 //Запускаем просиотор видео
	ShellExecute(Handle,
	L"open", (ExtractFilePath(Application->ExeName) + L"Video\\Chapters\\Chapter_2\\Отношения.mp4").w_str(),
	NULL, NULL, SW_SHOWNORMAL);
}
//---------------------------------------------------------------------------
...
void __fastcall TForm1::Imag_14Click(TObject *Sender)
{
	//Запускаем просиотор видео
	ShellExecute(Handle,
	L"open", (ExtractFilePath(Application->ExeName) + L"Video\\Chapters\\Chapter_2\\Специальные свойства отношений.mp4").w_str(),
	NULL, NULL, SW_SHOWNORMAL);
}
//---------------------------------------------------------------------------
...
void __fastcall TForm1::Imag_15Click(TObject *Sender)
{
	//Запускаем просиотор видео
	ShellExecute(Handle,
	L"open", (ExtractFilePath(Application->ExeName) + L"Video\\Chapters\\Chapter_2\\Эквивалентность и порядок.mp4").w_str(),
	NULL, NULL, SW_SHOWNORMAL);
}
//---------------------------------------------------------------------------
...
void __fastcall TForm1::Imag_16Click(TObject *Sender)
{
	//Запускаем просиотор видео
	ShellExecute(Handle,
	L"open", (ExtractFilePath(Application->ExeName) + L"Video\\Chapters\\Chapter_2\\Функции (отображения).mp4").w_str(),
	NULL, NULL, SW_SHOWNORMAL);
}
//---------------------------------------------------------------------------
...
void __fastcall TForm1::Imag_20Click(TObject *Sender)
{
	//Запускаем просиотор видео
	ShellExecute(Handle,
	L"open", (ExtractFilePath(Application->ExeName) + L"Video\\Chapters\\Chapter_3\\Высказывания и высказывательные формы.mp4").w_str(),
	NULL, NULL, SW_SHOWNORMAL);
}
//---------------------------------------------------------------------------
...
void __fastcall TForm1::Imag_21Click(TObject *Sender)
{
	//Запускаем просиотор видео
	ShellExecute(Handle,
	L"open", (ExtractFilePath(Application->ExeName) + L"Video\\Chapters\\Chapter_3\\Пропозициональные логические связки.mp4").w_str(),
	NULL, NULL, SW_SHOWNORMAL);
}
//---------------------------------------------------------------------------
...
void __fastcall TForm1::Imag_22Click(TObject *Sender)
{
	//Запускаем просиотор видео
	ShellExecute(Handle,
	L"open", (ExtractFilePath(Application->ExeName) + L"Video\\Chapters\\Chapter_3\\Язык логики высказываний.mp4").w_str(),
	NULL, NULL, SW_SHOWNORMAL);
}
//---------------------------------------------------------------------------
...
void __fastcall TForm1::Imag_23Click(TObject *Sender)
 {
	//Запускаем просиотор видео
	ShellExecute(Handle,
	L"open", (ExtractFilePath(Application->ExeName) + L"Video\\Chapters\\Chapter_3\\Тавтологии.mp4").w_str(),
	NULL, NULL, SW_SHOWNORMAL);
}
//---------------------------------------------------------------------------
...
void __fastcall TForm1::Imag_24Click(TObject *Sender)
{
	//Запускаем просиотор видео
	ShellExecute(Handle,
	L"open", (ExtractFilePath(Application->ExeName) + L"Video\\Chapters\\Chapter_3\\Равносильности.mp4").w_str(),
	NULL, NULL, SW_SHOWNORMAL);
}
//---------------------------------------------------------------------------
...
void __fastcall TForm1::Image4Click(TObject *Sender)
{
	//Запускаем просиотор видео
	ShellExecute(Handle,
	L"open", (ExtractFilePath(Application->ExeName) + L"Video\\Chapters\\Chapter_4\\Предикаты и кванторы.mp4").w_str(),
	NULL, NULL, SW_SHOWNORMAL);
}
//---------------------------------------------------------------------------
...
void __fastcall TForm1::Imag_29Click(TObject *Sender)
{
	//Запускаем просиотор видео
	ShellExecute(Handle,
	L"open", (ExtractFilePath(Application->ExeName) + L"Video\\Chapters\\Chapter_4\\Термы и формулы.mp4").w_str(),
	NULL, NULL, SW_SHOWNORMAL);
}
//---------------------------------------------------------------------------
...
void __fastcall TForm1::Imag_30Click(TObject *Sender)
{
	//Запускаем просиотор видео
	ShellExecute(Handle,
	L"open", (ExtractFilePath(Application->ExeName) + L"Video\\Chapters\\Chapter_4\\Интерпретация формул.mp4").w_str(),
	NULL, NULL, SW_SHOWNORMAL);
}
//---------------------------------------------------------------------------
...
void __fastcall TForm1::Imag_31Click(TObject *Sender)
{
    //Запускаем просиотор видео
	ShellExecute(Handle,
	L"open", (ExtractFilePath(Application->ExeName) + L"Video\\Chapters\\Chapter_4\\Формулы общезначимые, выполнимые, логически эквивалентные.mp4").w_str(),
	NULL, NULL, SW_SHOWNORMAL);
}
//---------------------------------------------------------------------------
...
void __fastcall TForm1::Imag_32Click(TObject *Sender)
{
    //Запускаем просиотор видео
	ShellExecute(Handle,
	L"open", (ExtractFilePath(Application->ExeName) + L"Video\\Chapters\\Chapter_4\\Перевод с естественного языка на логический и обратно.mp4").w_str(),
	NULL, NULL, SW_SHOWNORMAL);
}
//---------------------------------------------------------------------------
...
void __fastcall TForm1::Imag_33Click(TObject *Sender)
{
    //Запускаем просиотор видео
	ShellExecute(Handle,
	L"open", (ExtractFilePath(Application->ExeName) + L"Video\\Chapters\\Chapter_4\\Примеры перевода с естественного языка на логический и обратно.mp4").w_str(),
	NULL, NULL, SW_SHOWNORMAL);
}
//---------------------------------------------------------------------------
...
{
	//Проводим результат тестирования
	...
	if(Test_Ball_1 >= 4)
	{
		...
		btnTest_01->Caption = "Хорошие новости, вы прошли!";
		Test_Rezult_1 = Test_Rezult_1 + Test_1;
        Final_Ball = Final_Ball + Test_Rezult_1;
	}
	else if(Test_Ball_1 < 4)
	{
		btnTest_01->Caption = "Пока не верно, но вы можете попробовать еще!";
		Sleep(2000);
		btnTest_01->Caption = "Проверить";
	}
}
//---------------------------------------------------------------------------
...
void __fastcall TForm1::btnTest_02Click(TObject *Sender)
{
	...
	if(Test_Ball_2 >= 4)
	{
		...
		btnTest_02->Caption = "Хорошие новости, вы прошли!";
		Test_Rezult_2 = Test_Rezult_2 + Test_2;
		Final_Ball = Final_Ball + Test_Rezult_2;
	}
	else if(Test_Ball_2 < 4)
	{
		btnTest_02->Caption = "Пока не верно, но вы можете попробовать еще!";
		Sleep(2000);
		btnTest_02->Caption = "Проверить";
	}
}
//---------------------------------------------------------------------------
...
void __fastcall TForm1::btnTest_03Click(TObject *Sender)
{
	...
	if(Test_Ball_3 >= 3)
	{
		...
		btnTest_03->Caption = "Хорошие новости, вы прошли!";
		Test_Rezult_3 = Test_Rezult_3 + Test_3;
		Final_Ball = Final_Ball + Test_Rezult_3;
	}
	else if(Test_Ball_3 < 3)
	{
		btnTest_03->Caption = "Пока не верно, но вы можете попробовать еще!";
		Sleep(2000);
		btnTest_03->Caption = "Проверить";
	}
}
//---------------------------------------------------------------------------
...
void __fastcall TForm1::btnTest_04Click(TObject *Sender)
{
	...
	if(Test_Ball_4 >= 4)
	{
		...
		btnTest_04->Caption = "Хорошие новости, вы прошли!";
		Test_Rezult_4 = Test_Rezult_4 + Test_4;
		Final_Ball  = Final_Ball + Test_Rezult_4;
	}
	else if(Test_Ball_4 < 4)
	{
		btnTest_04->Caption = "Пока не верно, но вы можете попробовать еще!";
		Sleep(2000);
		btnTest_04->Caption = "Проверить";
	}
}
//---------------------------------------------------------------------------
...
{
	if(btnDeleteYourAccount->Caption == "Удалить учетную запись")
	{
		pnlProfile->BringToFront();
		pnlProfile->Enabled = true;
		pnlProfile->Visible = true;
		...
		btnDeleteYourAccount->Caption = "Отменить удаление";
	}
	else if(btnDeleteYourAccount->Caption == "Отменить удаление")
	{
		pnlConfirmation->Enabled = false;
		pnlConfirmation->Visible = false;
		btnDeleteYourAccount->Caption = "Удалить учетную запись";
...
{
	if(btnResetDefaultCourse->Caption == "Сбросить курс по умолчанию")
	{
        pnlProfile->BringToFront();
		pnlProfile->Enabled = true;
		pnlProfile->Visible = true;
		...
		btnResetDefaultCourse->Caption = "Отменить сброс курса";

	}
	else if(btnResetDefaultCourse->Caption == "Отменить сброс курса")
	{
		pnlConfirmation_2->Enabled = false;
		pnlConfirmation_2->Visible = false;

		btnResetDefaultCourse->Caption = "Сброс курса по умолчанию";
	}
}
//---------------------------------------------------------------------------
...
{
   //Поиск существующих тем для изучения
	UnicodeString Text = sb_Classes_Search->Text;

	for(int i=1;i<=36;i++)
	{
		TComponent *компонент;
		if(i<10)компонент=FindComponent("pnlTopic_0"+IntToStr(i)); //ищем компонент с нужным нам именем
		else компонент=FindComponent("pnlTopic_"+IntToStr(i)); //ищем компонент с нужным нам именем
		if(компонент!=NULL) //если компонент с таким именем найден на форме
		{
			if(Text != "")
			{
				int позиция=AnsiPos(Text,((TPanel*)компонент)->Caption);
				if(позиция>0)((TPanel*)компонент)->Visible=true;
				else ((TPanel*)компонент)->Visible=false;
			}
			else
			{
				((TPanel*)компонент)->Visible=true;
			}
		}
	}
}
...
{
   //Блокируем все символы кроме допустимых
	UnicodeString Text = "АБВГДЖЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯабвгдеёжзийклмнопрстуфхцчшщъыьэюя";
	int j = 0;
